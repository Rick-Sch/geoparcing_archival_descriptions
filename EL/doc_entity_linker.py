...
class DocEntityLinker():
  ...
  def __init__(self, kb_types=None, kb_df_embed_dict=None, TypesSubset=None, 
    ...
    """
    Doc Entity Linker for linking description locations to Wikidata, suitable to use as 
    custom component in SpaCy pipeline. Links accessable through `doc._.links`
  ...
  def __call__(self, doc):
    """
    Link entities from a description to wikidata items, and save in SpaCy Doc object. 
    Wapper around LinkEntities
    """

    Doc.set_extension("links", default={}, force=True)
    
    locs = [(ent.text, ent.label_) for ent in doc.ents]
    
    doc._.links = self.LinkEntities(doc.text, locs)
